...
for (const route of ALL_CANONICAL_ROUTES) {
  ...
  html = html.replace(/<meta property="og:url" content=".*?" \/>/s, `<meta property="og:url" content="${meta.canonicalUrl}" />`);
  ...
  if (meta.schemas && meta.schemas.length > 0) {
    const schemaScript = `\n    <!-- Schema.org Structured Data -->\n    <script type="application/ld+json">\n${JSON.stringify(
      meta.schemas.length === 1 ? meta.schemas[0] : { "@context": "https://schema.org", "@graph": meta.schemas },
      null,
      2
    )}\n    </script>\n  </head>`;
  ...
  if (route.path === '/') {
    ...
    // e.g. /services/ac-repair -> dist/services/ac-repair/index.html
...
console.log(`✓ Prerendered ${generatedCount} static HTML pages with unique metadata and JSON-LD schemas.`);

// 2. Generate dist/404.html (SPA client-side fallback)
const notFoundHtml = templateHtml
  .replace(/<title>.*?<\/title>/s, `<title>Page Not Found (404) | ${SITE_CONFIG.brandName}</title>`)
...
console.log('✓ Generated dist/404.html with noindex directive for GitHub Pages SPA fallback.');

// 3. Generate XML Sitemap
const sitemapEntries = ALL_CANONICAL_ROUTES.map(route => {
  const cleanPath = route.path.startsWith('/') ? route.path : `/${route.path}`;
  const loc = `${SITE_CONFIG.siteUrl}${cleanPath === '/' ? '/' : cleanPath}`;
  return `  <url>
    <loc>${loc}</loc>
...
console.log(`✓ Generated dist/sitemap.xml with ${ALL_CANONICAL_ROUTES.length} canonical URLs.`);
