...
**Project**: Vetrigaram Tech Services  
**Repository**: `vasanth-software-dev/Vetrigaram_services`  
**Deployment**: GitHub Pages (`https://vasanth-software-dev.github.io/Vetrigaram_services/`)  
...
The enterprise SEO upgrade transforms Vetrigaram from a basic single-page hash-scrolling app into a multi-route, static-prerendered, Schema.org-enriched home-services portal. The architecture consists of four distinct layers:
...
- All canonical URLs are generated as **absolute HTTPS URLs** anchored to `https://vasanth-software-dev.github.io/Vetrigaram_services/`.
- The root URL strictly uses a trailing slash (`https://vasanth-software-dev.github.io/Vetrigaram_services/`).
- Sub-routes strictly omit trailing slashes (`https://vasanth-software-dev.github.io/Vetrigaram_services/services/ac-repair`).
...
| **`Service`** | `/services/:serviceSlug`, `/:location/:service` | Service name, category, description, provider (`Vetrigaram Tech Services`), areaServed, and transparent price offer (`₹149 Inspection`). |
...
Vetrigaram operates primarily in the Chennai metropolitan area and the Ambattur industrial/residential corridor. The local SEO strategy avoids spammy doorway pages by delivering genuine, differentiated local content:
...
Sitemap: https://vasanth-software-dev.github.io/Vetrigaram_services/sitemap.xml
```
...
1. **Domain / URL Prefix Property**:
   - In Google Search Console, add property: `https://vasanth-software-dev.github.io/Vetrigaram_services/`.
...
4. **URL Inspection**:
   - Test `https://vasanth-software-dev.github.io/Vetrigaram_services/services/ac-repair` in the GSC URL Inspection tool.
...
| **Phase 2** | **Custom Domain Setup**: Migrate from `vasanth-software-dev.github.io/Vetrigaram_services` to a custom brand domain (e.g. `Vetrigaramservices.com`). | Boosts domain authority, brand memorability, and click-through rates. | High |
